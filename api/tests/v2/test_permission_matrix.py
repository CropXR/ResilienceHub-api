...
class InvestigationPermissionMatrixV2Test(PermissionMatrixTestBase):
    """Test the permission matrix for investigations in v2 API"""
    ...
    def test_api_visibility(self):
        """Test that API properly respects visibility restrictions"""
        # Login as different user types and check access
        
        # Create a restricted investigation 
        restricted = Investigation.objects.create(
        ...
        # Add owner and authorized user
        restricted.assign_role(self.owner_user, 'owner')
        restricted.assign_role(self.authorized_user, 'authorized')
        
        # Test API access as guest
        self.client.login(username='guest', password='password')
        response = self.client.get(f'/api/v2/investigations/{restricted.accession_code}/')
        self.assertIn(response.status_code, [403, 404])  # Should be forbidden or not found
        
        # Test API access as authorized user
        self.client.login(username='authorized', password='password')
        response = self.client.get(f'/api/v2/investigations/{restricted.accession_code}/')
        self.assertEqual(response.status_code, 200, "Authorized users should be able to access restricted investigations")
        
        # Test API access as owner
        self.client.login(username='owner', password='password')
        response = self.client.get(f'/api/v2/investigations/{restricted.accession_code}/')
        self.assertEqual(response.status_code, 200, "Owners should be able to access restricted investigations")

class StudyPermissionMatrixV2Test(PermissionMatrixTestBase):
    """Test the permission matrix for studies in v2 API"""
    ...
    def test_api_visibility(self):
        ...
        # Create a restricted study 
        restricted = Study.objects.create(
            investigation=self.investigation,
        ...
        # Test API access as guest
        self.client.login(username='guest', password='password')
        response = self.client.get(f'/api/v2/studies/{restricted.accession_code}/')
        self.assertIn(response.status_code, [403, 404])  # Should be forbidden or not found
        
        # Test API access as authorized user
        self.client.login(username='authorized', password='password')
        response = self.client.get(f'/api/v2/studies/{restricted.accession_code}/')
        self.assertEqual(response.status_code, 200, "Authorized users should be able to access restricted studies")
        
        # Test API access as owner
        self.client.login(username='owner', password='password')
        response = self.client.get(f'/api/v2/studies/{restricted.accession_code}/')
        self.assertEqual(response.status_code, 200, "Owners should be able to access restricted studies")