...
class APIClient:
    ...
    def login(self, username: str, password: str) -> str:
        ...
        url = f"{self.base_url}/token/"  # No trailing slash in flat style
    ...
    def get_entity_details(self, accession_code: str) -> dict:
        ...
        url = f"{self.base_url}/{accession_code}"  # Flat URL
        try:
    ...
    def get_all_investigations(self, page=1) -> dict:
        ...
        url = f"{self.base_url}/investigations?page={page}"  # Flat URL
        try:
            response = self.session.get(url)
            if response.status_code == 200:
                return response.json()
...
def lookup_entity(accession, page=1):
    ...
    # Flat URL approach for entity lookups
    ...
    # Use the details to display or paginate related entities
    # For example, if it's an investigation, retrieve its studies and paginate them
